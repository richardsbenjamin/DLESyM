...
MODULE_DIR="/home/benjamin/DLESyM"
# set to -1 to use CPU
DEVICE_NUMBERS="0"
...
OUTPUT_FILE="${CACHE_DIR}/1week_forecast.out"
# Path to models 
...
for (( i=0; i<${LEN}; i++ ));
do
    RUN_CMD="python scripts/coupled_forecast_hdf5_cache_only.py \
        --atmos-model-path ${ATMOS_MODEL} \
        --ocean-model-path ${OCEAN_MODEL} \
        --non-strict \
        --forecast-init-start ${INIT_STARTS} \
        --forecast-init-end ${INIT_ENDS} \
        --freq ${FREQ} \
